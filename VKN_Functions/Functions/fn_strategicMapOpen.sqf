/*
	Author: Karel Moricky (Modifed by Curious)
...
VKN_fnc_strategicMapOpen_showIconText = _showIconText;
VKN_fnc_strategicMapOpen_missionIcon = _missionIcon;

//--- Calculate terrain size and outside color
_mapSize = worldSize;
VKN_fnc_strategicMapOpen_mapSize = _mapSize;
VKN_fnc_strategicMapOpen_isNight = _isNight;
...
VKN_fnc_strategicMapOpen_player = player;
//selectnoplayer;

//--- Process ORBAT
VKN_fnc_strategicMapOpen_ORBAT = [];
_onClick = [];
{
	...
	//--- Group size
	//_sizeLocal = _size max 0 min (count (VKN_fnc_ORBATGetGroupParams_sizes) - 1);
	//_sizeParams = VKN_fnc_ORBATGetGroupParams_sizes select _sizeLocal;
	//_sizeTexture = _sizeParams select 0;

	VKN_fnc_strategicMapOpen_ORBAT set [
		count VKN_fnc_strategicMapOpen_ORBAT,
		[
			_class,
...
VKN_fnc_strategicMapOpen_ORBATonClick = _onClick;

//--- Process Missions
VKN_fnc_strategicMapOpen_missions = [];
if (count _missions > 0) then {
	...
	_ctrlMissions lbsetcolor [_lbadd,[1,1,1,0.5]];
	{
		...
		VKN_fnc_strategicMapOpen_missions set [
	...
	_ctrlMissions ctrladdeventhandler [
		...
		"
			...
			_mission = VKN_fnc_strategicMapOpen_missions select (_ctrlMissions lbvalue _cursel);
			[_mission select 0,1] spawn VKN_fnc_strategicmapanimate;
		"
	];
};

//--- Process Images
VKN_fnc_strategicMapOpen_images = [];
{
	...
	VKN_fnc_strategicMapOpen_images set [
		count VKN_fnc_strategicMapOpen_images,
...
VKN_fnc_strategicMapOpen_overcast = _overcast;
VKN_fnc_strategicMapOpen_clouds = [];

for "_i" from 1 to (_cloudsMax) do {
	VKN_fnc_strategicMapOpen_clouds set [
		count VKN_fnc_strategicMapOpen_clouds,
		[
...
VKN_fnc_strategicMapOpen_indexSizeTexture =	("sizeTexture" call bis_fnc_ORBATGetGroupParams);
VKN_fnc_strategicMapOpen_indexTextureSize =	("textureSize" call bis_fnc_ORBATGetGroupParams);

VKN_fnc_strategicMapOpen_draw = {
	scriptname "VKN_fnc_strategicMapOpen - Draw";
	_map = _this select 0;
	_mapSize = VKN_fnc_strategicMapOpen_mapSize / 2;
	...
	_mousePos = _map ctrlmapscreentoworld VKN_fnc_strategicMapOpen_mousePos;
	//_mouseLimit = VKN_fnc_strategicMapOpen_mapSize / 3400;
	...
	{
		_map drawicon _x;
	} foreach VKN_fnc_strategicMapOpen_images;
	...
	//--- ORBAT groups
	{
		...
		_textureSize = _classParams select VKN_fnc_strategicMapOpen_indexTextureSize;
		...
		if (_size >= 0) then {
			_sizeTexture = _classParams select VKN_fnc_strategicMapOpen_indexSizeTexture;
			_iconSizeParams set [0,_sizeTexture];
			_map drawIcon _iconSizeParams;
		};

	} foreach VKN_fnc_strategicMapOpen_ORBAT;

	//--- Clouds
	_cloudSpeed = sin _time * (1138 + 2000 * VKN_fnc_strategicMapOpen_overcast);
	{
		...
		_map drawIcon [
			...
			_size,
			_size,
			_dir + (_time * _foreachindex) / (count VKN_fnc_strategicMapOpen_clouds * 3),
			"",
			false
		];
	} foreach VKN_fnc_strategicMapOpen_clouds;
	...
	_textureAnimPhase = abs(6 - floor (_time * 16) % 12);
	{
		...
		_dir = 0;
		_alpha = 0.75;
		_texture = format [VKN_fnc_strategicMapOpen_missionIcon,_textureAnimPhase,_foreachindex + 1];
		...
		_map drawIcon [
			...
			_size,
			_dir,
			if (VKN_fnc_strategicMapOpen_showIconText) then {" " + _title} else {""},
			2,
			0.08,
			"PuristaBold"
		];
	} foreach VKN_fnc_strategicMapOpen_missions;

	//--- Night
	if (VKN_fnc_strategicMapOpen_isNight) then {
		_map drawRectangle [
			[_mapSize,_mapSize,0],
			99999,
			99999,
	...
	//--- Tooltip
	if (count _selected > 0 && !VKN_fnc_strategicMapOpen_mouseClickDisable) then {
		switch (count _selected) do {
			...
			case 4: {
				...
				[_classParams,_display,VKN_fnc_strategicMapOpen_mousePos] call bis_fnc_ORBATTooltip;
			};

			//--- Mission
			case 10;
			case 9: {
				[_selected,_display,VKN_fnc_strategicMapOpen_mousePos] call bis_fnc_ORBATTooltip;
			};
		};
	} else {
	...
	_info ctrlcommit 0;
	VKN_fnc_strategicMapOpen_selected = _selected;
};

//--- Mouse click on icon
VKN_fnc_strategicMapOpen_mouseClickDisable = false;
VKN_fnc_strategicMapOpen_selected = [];
VKN_fnc_strategicMapOpen_mousePos = [0,0];
VKN_fnc_strategicMapOpen_mouse = {
	VKN_fnc_strategicMapOpen_mousePos = [_this select 1,_this select 2];
};

#define DIK_H               0x23
#define DIK_NUMPAD5         0x4C

VKN_fnc_strategicMapOpen_keyDown = {
	...
	switch _key do {
		...
		case DIK_NUMPAD5: {
			[VKN_fnc_strategicMapOpen_mapCenter,1] spawn bis_fnc_strategicMapAnimate;
		};
	};
	false
};
...
VKN_fnc_strategicMapOpen_mapCenter = _mapCenter;

_map ctrladdeventhandler ["draw","_this call VKN_fnc_strategicMapOpen_draw;"];
_map ctrladdeventhandler ["mousemoving","_this call VKN_fnc_strategicMapOpen_mouse;"];
_map ctrladdeventhandler ["mouseholding","_this call VKN_fnc_strategicMapOpen_mouse;"];
_map ctrladdeventhandler ["mousebuttonclick","[nil,_this] spawn VKN_fnc_strategicMapMouseButtonClick;"];
_display displayaddeventhandler ["keydown","_this call VKN_fnc_strategicMapOpen_keyDown"];
...
VKN_fnc_strategicMapOpen_markers = _markers;
...
VKN_fnc_strategicMapOpen_camera = if (count allmissionobjects "Camera" == 0) then {
...
_display displayaddeventhandler [
	...
	"
		{
			_x setmarkeralpha 0;
		} foreach VKN_fnc_strategicMapOpen_markers;

		VKN_fnc_strategicMapOpen_camera cameraeffect ['terminate','back'];
		camdestroy VKN_fnc_strategicMapOpen_camera;

		VKN_fnc_strategicMapOpen_camera = nil;
		VKN_fnc_strategicMapOpen_player = nil;
		VKN_fnc_strategicMapOpen_mapSize = nil;
		VKN_fnc_strategicMapOpen_mapCenter = nil;
		VKN_fnc_strategicMapOpen_isNight = nil;
		VKN_fnc_strategicMapOpen_ORBAT = nil;
		VKN_fnc_strategicMapOpen_ORBATOverlay = nil;
		VKN_fnc_strategicMapOpen_missions = nil;
		VKN_fnc_strategicMapOpen_markers = nil;
		VKN_fnc_strategicMapOpen_images = nil;
		VKN_fnc_strategicMapOpen_draw = nil;
		VKN_fnc_strategicMapOpen_clouds = nil;
		VKN_fnc_strategicMapOpen_mousePos = nil;
		VKN_fnc_strategicMapOpen_mouseClickDisable = nil;
		VKN_fnc_strategicMapOpen_selected = nil;
		VKN_fnc_strategicMapOpen_indexSizeTexture = nil;
		VKN_fnc_strategicMapOpen_indexTextureSize = nil;
		with uinamespace do {
